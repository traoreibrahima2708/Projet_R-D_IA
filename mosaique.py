...
class Mosaique:
    ...
    def __init__(self,root):
        ...
        mos = tk.Toplevel(root)
        mos.title("Mosaique d'image")
        mos.geometry("%dx%d" % (root.winfo_screenwidth(), root.winfo_screenheight()))
        ...
        mosaic(filename, '/Users/vince/Desktop/Demonstrateur/Photos/IMAGE')

        im = Image.open(filename)
        im = im.resize((700, 700), Image.ANTIALIAS)
        imOrig = ImageTk.PhotoImage(im)

        im2 = Image.open(OUT_FILE)
        im2 = im2.resize((700, 700), Image.ANTIALIAS)
        im2F = ImageTk.PhotoImage(im2)


        can = Canvas(mos, bg="white")
        can.pack(side=tk.LEFT, fill=BOTH, expand=True)
        can.image = imOrig
        can.create_image(0, 0, anchor=NW,
                          image=imOrig)

        can_mos = Canvas(mos, bg="white")
        can_mos.pack(side=tk.LEFT, fill=BOTH, expand=True)
        can_mos.image = im2F
        can_mos.create_image(0, 0, anchor=NW,
                         image=im2F)
